...
class MiningSubscription(Subscription):
    ...
    @classmethod
    def on_template(cls, hashutxoroot, hashstateroot, job_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, clean_jobs):
        '''Push new job to subscribed clients'''
        cls.last_broadcast = (job_id, hashutxoroot, hashstateroot, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, clean_jobs)
        cls.emit(job_id, hashutxoroot, hashstateroot, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, clean_jobs)
        
    def _finish_after_subscribe(self, result):
        '''Send new job to newly subscribed client'''
        try:        
            (job_id, hashutxoroot, hashstateroot, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, _) = self.last_broadcast
        except Exception:
            log.error("Template not ready yet")
            return result
        
        self.emit_single(job_id, hashutxoroot, hashstateroot, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, True)
        return result
